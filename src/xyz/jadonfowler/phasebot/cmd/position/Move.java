...
public class Move extends Command {
	...
	public void exec(String in, String[] args, Session s) {
		...
		if (args[1].contains("*"))
			rx = Double.parseDouble(args[1].replace("*", "")) - PhaseBot.getBot().pos.x;
		else
			rx = Double.parseDouble(args[1]);

		if (args[2].contains("*"))
			ry = Double.parseDouble(args[2].replace("*", "")) - PhaseBot.getBot().pos.y;
		else
			ry = Double.parseDouble(args[2]);

		if (args[3].contains("*"))
			rz = Double.parseDouble(args[3].replace("*", "")) - PhaseBot.getBot().pos.z;
		else
			rz = Double.parseDouble(args[3]);
