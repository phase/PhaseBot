...
public class Set extends Command {
	...
	public void exec(String in, String[] args, Session s) {
		...
		case "p0":
			...
			s.send(
					new ClientChatPacket("p0 is set to " + (int) PhaseBot.getBot().pos.x + " " + (int) PhaseBot.getBot().pos.y
							+ " " + (int) PhaseBot.getBot().pos.z));
			break;
		case "p1":
			PhaseBot.getBot().positions[1] = PhaseBot.getBot().pos;
			s.send(
					new ClientChatPacket("p1 is set to " + (int) PhaseBot.getBot().pos.x + " " + (int) PhaseBot.getBot().pos.y
							+ " " + (int) PhaseBot.getBot().pos.z));
			break;
		case "p2":
			PhaseBot.getBot().positions[2] = PhaseBot.getBot().pos;
			s.send(
					new ClientChatPacket("p2 is set to " + (int) PhaseBot.getBot().pos.x + " " + (int) PhaseBot.getBot().pos.y
							+ " " + (int) PhaseBot.getBot().pos.z));
			break;
		case "p3":
			PhaseBot.getBot().positions[3] = PhaseBot.getBot().pos;
			s.send(
					new ClientChatPacket("p3 is set to " + (int) PhaseBot.getBot().pos.x + " " + (int) PhaseBot.getBot().pos.y
							+ " " + (int) PhaseBot.getBot().pos.z));
			break;
		}
	}
