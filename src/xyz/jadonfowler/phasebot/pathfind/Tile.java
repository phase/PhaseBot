...
public class Tile {
    ...
    public Vector3d getLocation(Vector3d start) {
        return new Vector3d(start.getBlockX() + x, start.getBlockY() + y, start.getBlockZ() + z);
    }
    ...
    public void calculateG(int sx, int sy, int sz, boolean update) {
        if ((!update && g == -1) || update) {
            ...
            while ((currentParent = currentTile.getParent()) != null) {
                // this probably shouldn't be a loop, stop at the end of the
                // loop, and add the parent's gCost.
                int dx = currentTile.getX() - currentParent.getX(), dy = currentTile.getY() - currentParent.getY(), dz = currentTile
                        .getZ() - currentParent.getZ();
                dx = abs(dx);
                dy = abs(dy);
                dz = abs(dz);
                if (dx == 1 && dy == 1 && dz == 1) {
